...
{
    uint8_t message;
    HAL_I2C_Mem_Read(&hi2c2, SLAVE_ADDRESS<<1, reg, I2C_MEMADD_SIZE_8BIT, &message, 1, 100);
    return message;
}
...
{
    uint8_t message[2];
    HAL_I2C_Mem_Read(&hi2c2, SLAVE_ADDRESS<<1, reg, I2C_MEMADD_SIZE_8BIT, message, 2, 100);
...
static void WriteToRegister(uint8_t reg, uint8_t value)
{
    HAL_I2C_Mem_Write(&hi2c2, SLAVE_ADDRESS<<1, reg, I2C_MEMADD_SIZE_8BIT, &value, 1, 100);
}
